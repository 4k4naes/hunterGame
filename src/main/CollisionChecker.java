...
public class CollisionChecker {
    ...
    private boolean isCollisionTile(int col, int row) {
        ...
        if (col >= 0 && col < gp.maxWorldCol && row >= 0 && row < gp.maxWorldRow) {
            return gp.tileM.collisionMap[col][row]; // Korzystaj z collisionMap w TileManager
        }
        return false; // Poza granicami mapy domyślnie brak kolizji
    }
    ...
    public int checkObject(Entity entity, boolean player) {
        ...
        for (int i = 0; i < gp.obj.length; i++) {
            if (gp.obj[i] != null) {
                ...
                // Reset pozycji solidArea do wartości domyślnych
